...
function Navbar({ searchTerms, setSearchTerms, user }) {
  ...
  return (
    <div className="flex gap-3 md:gap-5   w-full mt-5 pd-7">
      <div className="flex justify-start items-center w-full  outline-none focus-within:shadow-lg">
        <IoMdSearch fontSize={21} className="ml-2"  />

        <input 
        type="text" 
        value={searchTerms}
        className=" p-2 ml-2 w-full  bg-white outline-none hover:border-2 hover:border-black rounded-md" 
      ...
      <div className="flex ">
        <Link to={`user-profile/${user?.id}`} className="hidden md:block">
          <img src={user?.picture} className="w-14 h-12 rounded-lg   " alt="" />
        </Link>
